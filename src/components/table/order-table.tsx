        ...
        <tbody>
          {orders.map((order) => (
            <tr key={order.id} className="hover:bg-zinc-50/70 transition-colors">
              <td className="text-left pl-5 font-medium text-zinc-500">
                #{order.id}
              </td>
              ...
              <td>
                <div
                  className={`${getColor(order.status)} inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full text-xs font-medium ring-1 ring-inset text-nowrap min-w-28 justify-around`}
                >
