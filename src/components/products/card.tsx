...
const Card: FC<Props> = ({ product }) => {
  const stockClass =
    product.stock > 25
      ? "g-emerald-50 text-emerald-700 ring-emerald-600/20"
      : product.stock > 0
    ...
    <div className="group card hover:shadow-lg transition duration-300 overflow-hidden hover:-translate-y-1">
      ...
      <div className="h-56 relative overflow-hidden bg-zinc-50">
        ...
        <div className="absolute top-3 right-3">
          <span
            className={`${stockClass} px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ring-inset`}
          >
      ...
      <div className="p-5">
        <div className="mb-3">
          <p className="text-zinc-500 font-medium uppercase tracking-wider mb-1">
            {product.brand}
          </p>
        ...
        <p className="text-zinc-600 text-sm mb-4 line-clamp-2 leading-relaxed h-11 ">
          {product.description}
        </p>
